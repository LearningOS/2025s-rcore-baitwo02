...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        let mut tasks = [TaskControlBlock {
            ...
            task_syscall_record: [0; MAX_SYSCALL_ID],
        }; MAX_APP_NUM];
...
impl TaskManager {
    ...
    /// When syscall oucrred, add the number of this syscall_id
    fn record_syscall_times(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_syscall_record[syscall_id] += 1;
        drop(inner);
    }

    /// Get the times of syscall_id
    fn get_syscall_times(&self, syscall_id: usize) -> u32 {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let syscall_times = inner.tasks[current].task_syscall_record[syscall_id];
        drop(inner);
        syscall_times
    }
}
